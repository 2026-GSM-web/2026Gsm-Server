...
- `APP_WEB_OAUTH_CALLBACK_REDIRECT_URL` — frontend URL the backend 302s to after login succeeds and the cookie is set. **Not yet finalized** — leave blank in `.env` until a real frontend URL is confirmed; do not guess a value. Blank means `GET /api/auth/dg/callback` fails fast with a 500 rather than silently redirecting nowhere.
