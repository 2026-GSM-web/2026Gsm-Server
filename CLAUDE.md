...
- `FRONTEND_ORIGIN` — CORS allowed origin(s) for the SPA, comma-separated if more than one (also used as the `Access-Control-Allow-Credentials` origin now that auth is cookie-based, see below). CORS requires an exact string match against the browser's `Origin` header (scheme/host/port), so this must match the frontend's real origin precisely — a stale or mismatched value here is a silent CORS failure (200 in the Network tab, but the browser blocks the response from JS; check the Console, not just Network, when diagnosing). Confirmed 2026-09-01: prod frontend is `https://service.https.gsmsv.site`, prod backend is `https://school.https.gsmsv.site` — different from the earlier `service.gsmsv.site:30812` value, so double-check the deployed `.env` (outside this repo, on the self-hosted runner) is current.
...
- `APP_WEB_OAUTH_CALLBACK_REDIRECT_URL` — frontend URL the backend 302s to after login succeeds and the cookie is set. Last confirmed 2026-08-24 (민욱): `https://2026-gsm-client.vercel.app/callback`. The frontend's prod domain has since moved to `https://service.https.gsmsv.site` (confirmed 2026-09-01, see `FRONTEND_ORIGIN` above), so this value is likely stale too — re-confirm the actual callback page URL with the frontend team before changing it; never fill it in from inference alone. Blank means `GET /api/auth/dg/callback` fails fast with a 500 rather than silently redirecting nowhere — that fallback stays in place for any deploy that hasn't set this yet.
